...
def ask_question(questions):
    ## Choose a random question and display/print it!
    question = random.choice(questions)
    print(question['question'])
    for option in question['options']:
        print(option)
    user_answer = input("enter your answer: \n") ## get the users answer
    is_correct = user_answer.lower() == question['answer'].lower() ## this converts both the question's answer and the user's answer to lowercase so it can see if its a clear match. The characters "A" and "a" have different unicode points and "user_answer == question['answer']" compares the two. Say, if the user answer was "A", it would have the Unicode point of U+0041, and if the question answer was a, it would have the unicode point of U+0061. U+0041 != U+0061 therefore the answer would be considered false, hence why ".lower" is used.

    ## the below two lines handle the printing of the question
    print("\n" + question['extra'])
    print("\n--------------------------\nPress a key to proceed\n--------------------------\n")
    
    getch.getch() ## getch is used to detect a user input.
